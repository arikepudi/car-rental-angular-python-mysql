...
# also requires server/.venv to exist (the e2e backend runs `source .venv/bin/activate`)

npx playwright test                              # whole suite, headed (a real Chromium window opens) — deliberate
npx playwright test tests/browse-and-filter.spec.ts   # one spec file
npx playwright test -g "overlap"                 # by title substring
npx playwright show-report                       # trace/screenshot/video for the most recent run's failures
# package.json also exposes: npm run test:e2e , test:e2e:debug , test:e2e:report
```

Needs `e2e/.env.test` (gitignored; `.env.test.example` has the placeholder) with `TEST_DATABASE_URL` — **a database that is not `server/.env`'s real `car_rental` database.** `e2e/global-setup.ts` refuses to run at all if the two URLs match, then truncates every table in the test database and reseeds it via `python -m app.seed` before every run. The suite starts its own frontend/backend instances on dedicated ports (`:4300`/`:8100` by default, see `e2e/playwright.config.ts`) so it never touches whatever dev servers are already running on `:4200`/`:8000`; the e2e `ng serve` uses its own `e2e/e2e-proxy.conf.json` (points `/api` at `:8100`), not `client/proxy.conf.json`, and the e2e backend gets `DATABASE_URL`/`SESSION_SECRET`/`CLIENT_ORIGIN` inline on its command line so `server/.env` is never read. `headless: false` is a deliberate, standing configuration choice, not a default left unconfigured — don't change it without the user asking. `e2e/` is never copied into the Docker image (see `Dockerfile`'s `COPY` lines) — it has no effect on what gets deployed.

**Performance/load-test suite (`perf/`)** — k6, run on demand before merging to validate this app's actual NFRs (not a generic load profile):

```bash
which k6 || echo "install from https://github.com/grafana/k6/releases (no npm package — standalone binary)"
./perf/run.sh                                    # safety check → reset+reseed → start backend on :8300 → k6 run → report
./perf/run.sh k6/scenarios/some-other-scenario.js  # run a different scenario file
```

Needs `perf/.env.loadtest` (gitignored; `.env.loadtest.example` has the placeholder) with `LOAD_TEST_DATABASE_URL` — **a database that is not `server/.env`'s real `car_rental` database**, checked the same way and for the same reason as `e2e/.env.test`, except more critical: a load test writes rows fast enough (thousands/minute at moderate TPS) that pointing it at the wrong database is an immediate flood, not a slow leak. `perf/scripts/reset_load_test_db.py` (run via `server/.venv`'s Python, not a separate perf-only venv) performs the same refuse-if-URLs-match check as the e2e suite, then truncates and reseeds. `perf/run.sh` also refuses to start if `LOAD_TEST_BACKEND_PORT` (`:8300` by default) is already occupied, and re-verifies after startup that the PID it launched is the one actually bound to that port — both checks exist because a stale leftover process on that port once caused a load-test run to silently test old code instead of what was actually on disk, which is exactly the kind of failure this whole suite exists to prevent elsewhere.

k6 hits the backend API directly (`http://localhost:8300/api/...`) — no Angular/frontend process is started, since k6 measures HTTP/backend throughput, not browser rendering. The current NFR-derived scenario (`perf/k6/scenarios/growth-ramp.js`) ramps from 20 TPS (today's baseline) to 60 TPS (3x growth headroom) over ~4 minutes, with per-transaction latency thresholds in `perf/k6/lib/config.js` (browse/detail/booking p95 targets, <1% error rate) — re-derive these from the user's actual stated NFRs if this app's real traffic targets ever change, don't just tune numbers until the suite passes. Reports (HTML via the pinned `k6-reporter` v3.0.4 + JSON, both timestamped) land in `perf/reports/` (gitignored). **Known real finding from the validation run**: `sign_in`'s p95 (~550–650ms) exceeds its 500ms threshold at 60 TPS, consistently reproduced — this is bcrypt's inherent per-call cost under load, not a test artifact; the suite is correctly reporting a real gap against the stated NFR, not something to "fix" by loosening the threshold.
...
- `schemas.py` — Pydantic models for the HTTP boundary only (request bodies, response shapes). Raw SQL is still the whole persistence story; Pydantic just validates/serializes what crosses the API edge.
...
The `perf/` suite was built via the `e2e-test-suite-performance-k6` Claude Code skill (`~/.claude/skills/e2e-test-suite-performance-k6/`) — read its reference docs before adding new scenarios (executor selection, tag-scoped thresholds, and the test-data-randomization/user-pool patterns in particular) or touching `run.sh`/`reset_load_test_db.py` (the load-test-DB-isolation and port-collision checks are non-negotiable). If this app's actual NFRs (TPS, concurrency, latency targets) ever change, re-run that skill's NFR interview rather than hand-editing `perf/k6/lib/config.js`'s numbers in isolation.
