...
- `routes/cars.py`, `routes/bookings.py`, `routes/auth_routes.py` — one router module per resource, each mounted with its own `/api/...` prefix in `main.py`. `GET /api/bookings/mine` requires auth and filters by `user_id` — keep it that way if it's ever refactored; an earlier sibling app of this same lineage shipped that endpoint unscoped by accident.
- `booking_logic.py` — `compute_total()` and `cancellation_terms()`. Both are meant to be called from route handlers, never duplicated client-side: pricing and cancellation eligibility/fee are always computed server-side, fresh, from the DB row.
